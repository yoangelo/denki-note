class SummariesController < ApplicationController
  def customer_month
    # MVPではテナントIDを固定
    tenant = Tenant.first
    return render json: { rows: [], total_hours: 0, amount_jpy: 0 } unless tenant
    ...
    # 顧客に紐づく現場のIDを取得
    site_ids = Site.where(tenant: tenant, customer_id: summary_params[:customer_id]).pluck(:id)

    # 現場に紐づく日報を取得
    reports = DailyReport.where(tenant: tenant, site_id: site_ids, work_date: month_range)
    report_map = reports.index_by(&:id)

    # 作業エントリを集計
    entries = WorkEntry.where(tenant: tenant, daily_report_id: reports.pluck(:id))
    ...
    # 金額計算
    customer = Customer.find_by(id: summary_params[:customer_id], tenant: tenant)
    tenant_setting = TenantSetting.find_by(tenant: tenant)
